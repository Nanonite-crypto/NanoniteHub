...
getgenv().NanoniteConnections = getgenv().NanoniteConnections or {}

local function HandleConnection(Connection: RBXScriptConnection, Name: string)
    if getgenv().NanoniteConnections[Name] then
        getgenv().NanoniteConnections[Name]:Disconnect()
    end

getgenv().NanoniteConnections[Name] = Connection
end
...
if queue_on_teleport then
    ...
    if typeof(TeleportData) == "table" and TeleportData.NanoniteRejoin then
        return
    end
...
task.spawn(function()
    ...
    while task.wait(60) do
        ...
        game:GetService("StarterGui"):SetCore("SendNotification", {
                Title = "A new Nanonite version has been detected!",
...
function CreateUniversalTabs()
	...
	getgenv().HandleConnection(Players.PlayerAdded:Connect(CheckIfStaff), "StaffJoin")
    ...
    Tab:CreateButton({
        Name = "⚙️ • Rejoin",
        Callback = function()
            TeleportService:Teleport(game.PlaceId, Player, {NanoniteRejoin = true})
        end,
    })
	...
	Notify("Welcome to Nanonite", `Loaded in {math.floor((tick() - StartLoadTime) * 10) / 10}s`, "loader-circle")
